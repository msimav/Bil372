 ...
 *
 */
public class LoginLogChart extends JFrame {
	 ...
	 */
	public LoginLogChart(final String title, int[][] arr) {
	...
	private CategoryDataset createDataset(int[][] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			dataset.addValue(arr[i][1], series1,msq.getUserName(categories[i]));
		}
	...
	private JFreeChart createChart(final CategoryDataset dataset) {
		...
		final JFreeChart chart = ChartFactory.createBarChart(
				"Login Sayýlarý",         // chart title
				null,               // domain axis label
				"Login Sayýlarý",                  // range axis label
				dataset,                  // data
	...
	public static void run() {
		msq.connect();
		int size = msq.getLoginId().length;
		int array2d[][] = new int[size][2];


		for(int i = 0 ; i < msq.getLoginId().length ;i++){

			array2d[i][0] = msq.getLoginId()[i];
			array2d[i][1] = msq.getLoginLog(msq.getLoginId()[i]); 
		}
		...
		LoginLogChart demo = new LoginLogChart("Ýstatistikler", array2d);
